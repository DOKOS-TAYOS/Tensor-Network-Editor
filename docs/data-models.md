...
```python
from tensor_network_editor import EdgeEndpointRef, HyperedgeSpec
...
In the editor, hyperedges are first-class saved objects in normal mode.
Generated backend code lowers them to autogenerated copy tensors plus binary
edges, because the target backends still consume pairwise connectivity.
