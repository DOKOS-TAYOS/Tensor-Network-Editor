...
4. Optionally create a hyperedge from three or more compatible open indices.
...
- every endpoint must be an open index, and all endpoints must share the same
  dimension
- the editor renders hyperedges as a visible hub plus spokes, but that hub is
  only a UI helper and is not saved as a tensor
