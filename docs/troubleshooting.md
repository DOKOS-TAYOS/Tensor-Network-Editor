...
`.npz` references, are also recovered. Editor-only contraction `view_snapshots`
are still dropped because the generated source does not encode that layout
state. Hyperedges are exported through autogenerated copy tensors, and
structured comments let the generated-source parser reconstruct the original
`HyperedgeSpec`.
