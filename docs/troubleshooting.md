...
round-trip. Supported tensor initializer modes, including external `.npy`,
`.npz`, and `.pt` references, are also recovered. Editor-only contraction
`view_snapshots` are still dropped because the generated source does not encode
that layout state. Hyperedges are exported through autogenerated copy tensors,
and structured comments let the generated-source parser reconstruct the
original `HyperedgeSpec`.
...
Current periodic generated Python embeds compact round-trip metadata, so linear,
grid, and tree periodic exports from current versions reload as editable
periodic-mode specs. Older periodic exports without that marker remain
output-only and still raise the older clear import error.
