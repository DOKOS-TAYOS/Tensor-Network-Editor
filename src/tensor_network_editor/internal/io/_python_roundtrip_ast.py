...
from ...models import TensorDataDType, TensorDataMode, TensorDataSpec
...
def _parse_tensor_data_initializer(
    ...
    external_data = _parse_external_tensor_data_initializer(call)
...
def _parse_external_tensor_data_initializer(
    call: ast.Call,
) -> tuple[tuple[int, ...], TensorDataSpec] | None:
    """Parse the generated external tensor-data helper call."""
    call_name = _call_name(call.func)
    if call_name.endswith(".as_tensor") or call_name == "as_tensor":
        if not call.args:
            return None
        parsed = _parse_external_tensor_data_load(call.args[0])
        if parsed is None:
            return None
        shape, tensor_data = parsed
        tensor_data.dtype = _parse_tensor_data_dtype_expression(
            _keyword_value(call, "dtype")
        )
        return shape, tensor_data

    if call_name.endswith(".astype") and isinstance(call.func, ast.Attribute):
        parsed = _parse_external_tensor_data_load(call.func.value)
        if parsed is None:
            return None
        shape, tensor_data = parsed
        tensor_data.dtype = _parse_tensor_data_dtype_expression(
    ...
    return _parse_external_tensor_data_load(call)


def _parse_external_tensor_data_load(
    expression: ast.expr,
) -> tuple[tuple[int, ...], TensorDataSpec] | None:
    """Parse ``_load_external_tensor_data(...)`` generated by codegen."""
    if not isinstance(expression, ast.Call):
        return None
    if _call_name(expression.func) != "_load_external_tensor_data":
        return None
    file_path_expression = _keyword_value(expression, "path")
    if file_path_expression is None and expression.args:
        file_path_expression = expression.args[0]
    file_path = _literal_string(file_path_expression)
    expected_shape = _literal_int_sequence(_keyword_value(expression, "expected_shape"))
    if file_path is None or expected_shape is None:
        return None
    array_key_expression = _keyword_value(expression, "array_key")
    array_key = _literal_string(array_key_expression)
    return (
        expected_shape,
        TensorDataSpec(
...
def _parse_tensor_data_dtype_expression(
    expression: ast.expr | None,
) -> TensorDataDType | None:
    """Parse a backend dtype expression emitted by codegen."""
    dtype_name = _call_name(expression) if expression is not None else ""
    return {
        "np.float32": TensorDataDType.FLOAT32,
        "np.float64": TensorDataDType.FLOAT64,
        "np.complex64": TensorDataDType.COMPLEX64,
        "np.complex128": TensorDataDType.COMPLEX128,
        "torch.float32": TensorDataDType.FLOAT32,
        "torch.float64": TensorDataDType.FLOAT64,
        "torch.complex64": TensorDataDType.COMPLEX64,
        "torch.complex128": TensorDataDType.COMPLEX128,
    }.get(dtype_name)
