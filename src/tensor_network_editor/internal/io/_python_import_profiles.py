...
class _ImportedTensor:
    """Tensor information recovered from one supported external AST node."""
...
class _ExplicitConnection:
    """One explicit binary connection recovered from source code."""
...
def _build_network_from_shared_labels(
    *,
    tensors_by_reference: dict[str, _ImportedTensor],
    tensor_order: list[str],
    allow_hyperedges: bool,
) -> NetworkSpec:
    """Build a ``NetworkSpec`` by inferring connections from shared labels."""
    tensor_specs, index_lookup = _build_imported_tensor_specs(
...
def _build_network_from_explicit_connections(
    *,
    tensors_by_reference: dict[str, _ImportedTensor],
    tensor_order: list[str],
    explicit_connections: list[_ExplicitConnection],
) -> NetworkSpec:
...
def _build_imported_tensor_specs(
    *,
    tensors_by_reference: dict[str, _ImportedTensor],
    tensor_order: list[str],
